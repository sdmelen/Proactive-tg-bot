...
class ExcelHandler:
    ...
    def generate_progress_prompt(self, progress: float) -> str:
        ...
        if 0 <= progress <= 20:
            return (
                f"Студент имеет прогресс {progress}%. Сгенерируй мотивирующее сообщение "
                "с легким подзатыльником, которое поможет ему начать активнее учиться. "
                "Важно быть дружелюбным, но настойчивым."
            )
        elif 20 < progress <= 50:
            return (
                f"Студент имеет прогресс {progress}%. Сгенерируй мотивирующее сообщение, "
            ...
            return (
                f"Студент имеет прогресс {progress}%. Сгенерируй ободряющее сообщение, "
                "подчеркивающее, что он молодец и уже близок к цели."
            )
        elif 80 < progress < 100:
            return (
                f"Студент имеет прогресс {progress}%. Сгенерируй воодушевляющее сообщение, "
